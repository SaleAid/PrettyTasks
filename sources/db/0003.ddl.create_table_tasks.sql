...
CREATE TABLE IF NOT EXISTS `tasks` (
  ...
  `datetime` datetime DEFAULT NULL COMMENT 'data and time for task',
  `checktime` bit(1) DEFAULT b'0' COMMENT 'Flag for check time; if not set, task has only date',
  `order` tinyint(4) DEFAULT '0' COMMENT 'order for sorting tasks',
  `done` bit(1) DEFAULT b'0' COMMENT 'Flag that task is done',
  `future` bit(1) DEFAULT b'0' COMMENT 'flag that task is for future',
  `repeatid` bit(1) DEFAULT b'0' COMMENT 'id of repeated task''s series',
